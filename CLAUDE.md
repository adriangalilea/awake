...
- CLI: `awake` · `awake 2h|90m|45` · `awake --until HH:MM` (next occurrence, tomorrow if past) · `awake -w PID` · `--display` · `asleep`/`awake off` · `awake status [--json]` (Status is Codable; the JSON form is the scripting surface) · `awake floor N` · `awake notify [CMD]` · `awake hotkey [COMBO]`. Fish aliases `caf`/`cof`/`coffee`/`amph` point at `awake`. End/engage replies carry the displaced session (`Reply.previous`) so breadcrumbs render from one round trip.
